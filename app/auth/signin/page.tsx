...
export default function SignInPage() {
  ...
  const { signInWithGoogle, loading } = useAuth();import { useState } from 'react';import { useState, Suspense } from 'react';

  const [error, setError] = useState<string | null>(null);

  const [signingIn, setSigningIn] = useState(false);import { useRouter } from 'next/navigation';import { useSearchParams } from 'next/navigation';
  ...
  const handleGoogleSignIn = async () => {
    ...
    setError(null);export default function SignInPage() {function SignInContent() {

    

    try {  const { signInWithGoogle, loading } = useAuth();  const [isLoading, setIsLoading] = useState(false);

      await signInWithGoogle();

    } catch (err) {  const [error, setError] = useState<string | null>(null);  const searchParams = useSearchParams();

      console.error('Sign in error:', err);

      setError('Failed to sign in. Please try again.');  const [signingIn, setSigningIn] = useState(false);  const callbackUrl = searchParams.get('callbackUrl') || '/dashboard';
  ...
  if (loading) {
    ...
    return (    setSigningIn(true);  const handleGoogleSignIn = async () => {

      <div className="min-h-screen flex items-center justify-center">

        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div>    setError(null);    setIsLoading(true);
  ...
  return (
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">      await signInWithGoogle();    } catch (error) {

      <div className="max-w-md w-full">

        <div className="text-center mb-8">    } catch (err) {      console.error('Sign in error:', error);

          <h1 className="text-4xl font-bold text-gray-900 mb-2">

            Welcome to LocalGrid      console.error('Sign in error:', err);      setIsLoading(false);

          </h1>

          <p className="text-gray-600">      setError('Failed to sign in. Please try again.');    }

            Sign in to connect with local skill providers

          </p>      setSigningIn(false);  };
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          {error && (  };  return (
            ...
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
              ...
              {error}    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
          ...
          <button    return (        <div className="text-center">

            onClick={handleGoogleSignIn}

            disabled={signingIn}      <div className="min-h-screen flex items-center justify-center">          <h1 className="text-4xl font-bold text-gray-900 mb-2">
          ...
          >        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div>            Welcome to LocalGrid
            ...
            {signingIn ? (
              ...
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-gray-900"></div>      </div>          </h1>
              ...
              <>    );          <p className="text-lg text-gray-600">
                ...
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  ...
                  <path  }            India&apos;s hyperlocal skill exchange platform
                  ...
                  <path  return (        </div>
                    ...
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">

                  />

                  <path      <div className="max-w-md w-full">        <div className="bg-white py-8 px-6 shadow-lg rounded-lg">

                    fill="#FBBC05"

                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"        <div className="text-center mb-8">          <div className="space-y-6">

                  />

                  <path          <h1 className="text-4xl font-bold text-gray-900 mb-2">            {error && (

                    fill="#EA4335"

                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"            Welcome to LocalGrid              <div className="p-4 bg-red-50 border border-red-200 rounded-lg">

                  />

                </svg>          </h1>                <p className="text-sm text-red-700">

                <span className="text-gray-700 font-medium">

                  Sign in with Google          <p className="text-gray-600">                  {error === 'OAuthAccountNotLinked'
          ...
          </button>          </p>                    : 'An error occurred during sign in. Please try again.'}



          <p className="mt-6 text-xs text-center text-gray-500">        </div>                </p>
